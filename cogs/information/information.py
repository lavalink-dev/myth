...
class Information(CogMeta):
    ...
    async def banner(self, ctx: Context, user: User = Author):
        try:
            user = await self.bot.fetch_user(user.id) # type: ignore
    ...
    async def shards(self, ctx):
        embed = discord.Embed(description=f"> Shard count: `{self.bot.shard_count}`", color=color.default)
        user_pfp = ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url
        ...
        for shard_id in range(self.bot.shard_count):
            ...
            guilds_shard = sum(1 for g in self.bot.guilds if g.shard_id == shard_id)
            users_shard = sum(g.member_count for g in self.bot.guilds if g.shard_id == shard_id)
