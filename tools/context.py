...
class Context(commands.Context):
    ...
    async def agree(self, message) -> discord.Message:
        embed = discord.Embed(description = f"> {emoji.agree} {self.author.mention}: {message}", color = color.agree)
        return await self.send(embed=embed)
    
    async def deny(self, message) -> discord.Message:
        embed = discord.Embed(description = f"> {emoji.deny} {self.author.mention}: {message}", color = color.deny)
        return await self.send(embed=embed)
    
    async def warn(self, message) -> discord.Message:
        embed = discord.Embed(description = f"> {emoji.warn} {self.author.mention}: {message}", color = color.warn)
        return await self.send(embed=embed)
    ...
    async def send_help(self, command_name: str):
        ...
        if command is None:
            embed = discord.Embed(description=f"{emoji.deny} {self.author.mention}: Could not **find** the command: `{command_name}`", color=color.deny)
            return await self.send(embed=embed)

        embed = discord.Embed(title=f"Command: {command.name}", description=f"> {command.description}" if command.description else '', color=color.default)
        ...
        if isinstance(command, commands.Group):
            ...
            for subcommand in subcommands:
                ...
                sub_embed = discord.Embed(title=f"Subcommand: {command.name} {subcommand.name}", description=f"> {subcommand.description}" if subcommand.description else '', color=color.default)
